...
void MotorDriverInit(sMotor_s *sMotor){
    ...
    Send2XMotorDriverEnable(sMotor, 0, 0);    /* enable motor driver */
    Send4XClrMotorEncoder(sMotor, 1, 1, 1, 1); /* send clear feedback counter */
    IOWR(MOTORDRIVERREG_BASE, 3, 0);    /* clear 03 register (MotorControl)*/
}
...
void MotorTestFun(sMotor_s *sMotor ){
    ...
    Send4XwheelPwm(sMotor, 10,0,0,0); /* send motor PWM duty cycle, 0~100 */
    Send2XMotorDriverEnable(sMotor,1, 1);  /* open motor driver, 1: enable, 0: disable */
    usleep(5000000);  /* delay time */
...
void ClearMotorSensorFifo(sMotor_s *sMotor, unsigned int uiME1, unsigned int uiME2,
                         unsigned int uiME3, unsigned int uiME4){
    IOWR(MOTORDRIVERREG_BASE, 3, 0x80000000); /* send [31] clear fifo command */
    usleep(1);
    IOWR(MOTORDRIVERREG_BASE, 3, 0x00000000); /* send [31] clear fifo command */
#ifdef DEBUG_MESSAGE
...
void Read4XMotorEncoder(sMotor_s *sMotor){
    ...
    sMotor->sMotorAp.iMotorFBEncoder = IORD(MOTORDRIVERREG_BASE, 7);
    sMotor->sMotorBp.iMotorFBEncoder = IORD(MOTORDRIVERREG_BASE, 12);
    sMotor->sMotorCp.iMotorFBEncoder = IORD(MOTORDRIVERREG_BASE, 17);
    sMotor->sMotorDp.iMotorFBEncoder = IORD(MOTORDRIVERREG_BASE, 22);
#ifdef DEBUG_MESSAGE
...
//                                     */
    ...
    if(uiFifostatus & 0x01){ /* check motor A speed fifo status */
        sMotor->sMotorAp.uiMotorFBSpeed = IORD(MOTORDRIVERREG_BASE, 8); /* motor A speed counter */
    }else{
    	sMotor->sMotorAp.uiMotorFBSpeed = 0xdeadbeef; /* no data */
    }
    if(uiFifostatus & 0x02){ /* check motor B speed fifo status */
        sMotor->sMotorBp.uiMotorFBSpeed = IORD(MOTORDRIVERREG_BASE, 13); /* motor A speed counter */
    }else{
    	sMotor->sMotorBp.uiMotorFBSpeed = 0xdeadbeef; /* no data */
    }
    if(uiFifostatus & 0x04){ /* check motor C speed fifo status */
        sMotor->sMotorCp.uiMotorFBSpeed = IORD(MOTORDRIVERREG_BASE, 18); /* motor A speed counter */
    }else{
    	sMotor->sMotorCp.uiMotorFBSpeed = 0xdeadbeef; /* no data */
    }
    if(uiFifostatus & 0x08){ /* check motor D speed fifo status */
        sMotor->sMotorDp.uiMotorFBSpeed = IORD(MOTORDRIVERREG_BASE, 23); /* motor A speed counter */
    }else{
