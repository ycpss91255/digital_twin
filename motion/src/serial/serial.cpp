...
void Serial::unbuild_msg() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    this->M_data[i].Speed = this->sub_msg[MOTOR_SPEED_ORDER + (i * 2) + 1]
                                << 8 |
                            this->sub_msg[MOTOR_SPEED_ORDER + (i * 2)];

    this->M_data[i].Voltage = this->sub_msg[MOTOR_VOLTAGE_ORDER + (i * 2)]
                                  << 8 |
                              this->sub_msg[MOTOR_VOLTAGE_ORDER + (i * 2)];

    this->M_data[i].Current = this->sub_msg[MOTOR_CURRENT_ORDER + (i * 2)]
                                  << 8 |
