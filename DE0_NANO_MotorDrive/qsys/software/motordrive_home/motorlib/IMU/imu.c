...
void ReadIMU(sIMU_s *sIMU){
    ...
    int uiIMUdata, iTmp;
    uiFifoStatus = IORD(MOTORDRIVERREG_BASE, 4); /* read sensor fifo register */
    if(uiFifoStatus & 0x100000) { /* 20th bit, Accelerometer fifo */
        uiIMUdata = IORD(MOTORDRIVERREG_BASE, 29); /* read accelerometer value*/
    ...
    if(uiFifoStatus & 0x200000){ /*21th bit, Gyroscope fifo */
        uiIMUdata = IORD(MOTORDRIVERREG_BASE, 30); /* read accelerometer value*/
...
//        printf("Gyro: x(%4d), Y(%4d), Z(%4d)\n",sIMU->iGyroX, sIMU->iGyroY, sIMU->iGyroZ);
    ...
    if(uiFifoStatus & 0x400000){ /*22th bit, Magnetic fifo */
        uiIMUdata = IORD(MOTORDRIVERREG_BASE, 31); /* read Magneticmeter value*/
